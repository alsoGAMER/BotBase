...
from BotBase.modules.start import back_start
from BotBase.config import CACHE, ADMINS, ADMINS_LIST_UPDATE_DELAY, callback_regex, admin_is_chatting, \
    ...
    CLOSE_CHAT_BUTTON, BACK_BUTTON, UPDATE_BUTTON, bot, \
    ADMIN_ACCEPTED_CHAT
...
def begin_chat(_, query):
    ...
    if query.from_user.id in ADMINS:
        ...
        for admin in ADMINS:
            ...
            if status != "IN_CHAT":
                message = wrapper.send_message(admin, SUPPORT_NOTIFICATION.format(uinfo=text),
...
def forward_from_admin(_, message):
    if message.text:
        logging.warning(
        ...
        wrapper.send_photo(CACHE[message.from_user.id][1], photo=message.photo.file_id, file_ref=message.photo.file_ref,
        ...
        wrapper.send_audio(CACHE[message.from_user.id][1], audio=message.audio.file_id, file_ref=message.audio.file_ref,
                           caption=ADMIN_MESSAGE.format(user_name=ADMINS[message.from_user.id],
                                                        user_id=NAME.format(message.from_user.id),
                                                        message=message.caption.html or '' if message.caption else ''))
    elif message.document:
        wrapper.send_document(CACHE[message.from_user.id][1], document=message.document.file_id,
                              file_ref=message.document.file_ref,
                              caption=ADMIN_MESSAGE.format(user_name=ADMINS[message.from_user.id],
                                                           user_id=NAME.format(message.from_user.id),
                                                           message=message.caption.html or '' if message.caption else ''))
    elif message.sticker:
        wrapper.send_sticker(CACHE[message.from_user.id][1], sticker=message.sticker.file_id,
                             file_ref=message.sticker.file_ref)
    elif message.video:
        wrapper.send_video(CACHE[message.from_user.id][1], video=message.video.file_id, file_ref=message.video.file_ref,
                           caption=ADMIN_MESSAGE.format(user_name=ADMINS[message.from_user.id],
                                                        user_id=NAME.format(message.from_user.id),
                                                        message=message.caption.html or '' if message.caption else ''))
    elif message.animation:
        wrapper.send_animation(CACHE[message.from_user.id][1], animation=message.animation.file_id,
                               file_ref=message.animation.file_ref,
                               caption=ADMIN_MESSAGE.format(user_name=ADMINS[message.from_user.id],
                                                            user_id=NAME.format(message.from_user.id),
                                                            message=message.caption.html or '' if message.caption else ''))
    elif message.voice:
        wrapper.send_voice(CACHE[message.from_user.id][1], voice=message.voice.file_id, file_ref=message.voice.file_ref,
                           caption=ADMIN_MESSAGE.format(user_name=ADMINS[message.from_user.id],
                                                        user_id=NAME.format(message.from_user.id),
                                                        message=message.caption.html or '' if message.caption else ''))
    elif message.video_note:
        wrapper.send_video_note(CACHE[message.from_user.id][1], video_note=message.video_note.file_id,
                                file_ref=message.video_note.file_ref)
    elif message.location:
        wrapper.send_location(CACHE[message.from_user.id][1], latitude=message.location.latitude,
                              longitude=message.location.longitude)
    elif message.contact:
        wrapper.send_contact(CACHE[message.from_user.id][1], phone_number=message.contact.phone_number,
                             first_name=message.contact.first_name, last_name=message.contact.last_name)
    elif message.poll:
        wrapper.forward_messages(CACHE[message.from_user.id][1], from_chat_id=message.chat.id,
                                 message_ids=message.message_id, as_copy=False)


@Client.on_message(user_is_chatting() & ~BANNED_USERS & ~user_banned())
def forward_from_user(_, message):
...
def join_chat(_, query):
    ...
    if CACHE[query.from_user.id][0] != "IN_CHAT":
        ...
        if CACHE[user_id][0] != "AWAITING_ADMIN":
            ...
            admin_joins = wrapper.send_message(user_id, ADMIN_JOINS_CHAT.format(admin_name=admin_name,
